...
const cookies = {
  ...
  reset() {
    ...
    console.info("Cookies existentes eliminadas")
    console.error("Se han eliminado las cookies esenciales")
    console.warn("Reinicio obligatorio de la página. Motivo: Página corrompida (Ausencia de cookies esenciales)")
...
function load() {
  ...
  for (x = 0; x < (Object.keys(navigation)).length; x++) {
    if (navigation[`${Object.keys(navigation)[x]}`].hidden) {
    } else if (navigation[`${x}`].class == "separador") {
      headerMenu[x] = `<div class="separador"><div class="vector">${navigation[`${x}`].vector}</div><div class="nombre"><p>${navigation[`${x}`].name}</p></div><div class="line"></div></div>`
      ...
      if ((navigation[`${x}`].name == "Cookies" || navigation[`${x}`].name == "Ajustes" || navigation[`${x}`].name == "Ajustes dev") && warned) {
        headerMenu[x] = `<div class="pagina disabled" title="El navegador no permite utilizar esta herramienta"><div class="vector">${navigation[`${x}`].vector}</div><div class="nombre"><p>${navigation[`${x}`].name}</p></div></div>`
      } else if (navigation[`${x}`].disabled) {
        headerMenu[x] = `<div class="pagina disabled" title="Página deshabilitada por el desarrollador"><div class="vector">${navigation[`${x}`].vector}</div><div class="nombre"><p>${navigation[`${x}`].name}</p></div></div>`
      } else {
        headerMenu[x] = `<div class="pagina" onclick="${navigation[`${x}`].onclick}" title="${navigation[`${x}`].title}"><div class="vector">${navigation[`${x}`].vector}</div><div class="nombre"><p>${navigation[`${x}`].name}</p></div></div>`
      }
    }
  }
